 ...
 */
SEACATCC_API int seacatcc_init(
...
SEACATCC_API int seacatcc_run(void);
SEACATCC_API int seacatcc_shutdown(void);
...
*/
SEACATCC_API int seacatcc_yield(char what);

SEACATCC_API const char * seacatcc_version(void);
...
SEACATCC_API void seacatcc_ppkgen_worker(void);
SEACATCC_API int seacatcc_csrgen_worker(const char * csr_entries[]);
...
SEACATCC_API int seacatcc_hook_register(char code, seacatcc_hook hook);
static inline int seacatcc_hook_unregister(char code) { return seacatcc_hook_register(code, NULL); }
...
SEACATCC_API void seacatcc_log(char level, const char * format, ...)  __attribute__ ((__format__ (__printf__, 2, 3)));

typedef void (* seacatcc_log_fnct)(char level, const char * message);
SEACATCC_API void seacatcc_log_setfnct(seacatcc_log_fnct log_fnct);
...
SEACATCC_API int seacatcc_log_set_mask(union seacatcc_log_mask_u mask);
...
SEACATCC_API int seacatcc_gwconn_cert(void * cert_buffer, uint16_t * cert_buffer_size);

SEACATCC_API const char * seacatcc_client_id(void);
SEACATCC_API const char * seacatcc_client_tag(void);

// State
#define SEACATCC_STATE_BUF_SIZE (7)
SEACATCC_API void seacatcc_state(char * state_buffer);


// Proxy
SEACATCC_API int seacatcc_set_proxy_server_worker(const char * proxy_host, const char * proxy_port);


// Discover
// Call this only prior seacatcc_run() and only once
SEACATCC_API void seacatcc_set_discover_domain(const char * domain);


// Sockets
// For unconfiguring a port, use: seacatcc_socket_configure_worker(0, [type], 0, [port number], NULL, NULL)
SEACATCC_API int seacatcc_socket_configure_worker(uint16_t port, uint16_t domain, uint16_t type, uint16_t protocol, const char * peer_address, const char * peer_port);


// Characteristics
SEACATCC_API int seacatcc_characteristics_store(const char ** characteristics);

// Misc
SEACATCC_API double seacatcc_time(void);
...
#define SEACATCC_RC_E_NOT_IMPLEMENTED (-9998)
