...
namespace Spring.Threading.Locks
{
	...
	public class ReentrantLock : ILock, IDisposable, ConditionVariable.IExclusiveLock
	{
		...
		private abstract class Sync
		{
			...
			{
				get { lock (this) return TaskHelper.CurrentTask == _owner; }
			}
            ...
            public bool TryLock()
			{
				Task caller = TaskHelper.CurrentTask;
			...
			public virtual ICollection<Task> QueuedThreads
			{
				get { throw new NotSupportedException("Use FAIR version"); }
			}

			public virtual bool IsQueued(Task thread)
			{
            ...
            protected bool GetHold(Task caller)
            {
                if (_owner == null)
                {
                    _owner = caller;
		...
		private sealed class NonfairSync : Sync
		{
            ...
            public override void Lock()
            {
                ...
                lock (this)
                {
                    ...
                    finally
                    {
                        if (wasInterrupted)
                        {
                            throw new Exception("Thread.CurerntThread.Interrupt doesn't exist");
                        }
                    }
                }
            }
		    ...
		    public override void Unlock()
			{
				lock (this)
				{
					if (TaskHelper.CurrentTask != _owner)
					{
        ...
        private sealed class FairSync : Sync, IQueuedSync
		{
			...
			public void TakeOver(WaitNode node)
			{
				lock (this)
				{
					Debug.Assert(_holds == 1 && _owner == TaskHelper.CurrentTask);
					_owner = node.Owner;
				}
			}
		    ...
		    private WaitNode GetSignallee(Task caller)
			{
				lock (this)
				{
					if (caller != _owner)
			...
			public override ICollection<Task> QueuedThreads
			{
				get
				{
					lock (this)
			...
			public override bool IsQueued(Task thread)
			{
				lock (this)
				{
		...
		/// </returns>
		protected internal virtual Task Owner
		{
		...
		public bool IsQueuedThread(Task thread)
		{
		...
		/// <exception cref="System.NullReferenceException">if the <paramref name="condition"/> is null</exception>
		/// <exception cref="System.ArgumentException">if the <paramref name="condition"/> is not associated with this lock</exception>
		protected virtual ICollection<Task> GetWaitingThreads(ICondition condition)
		{
		...
		{
			Task o = Owner;
			return base.ToString() + ((o == null) ? "[Unlocked]" : "[Locked by thread " + o.Id + "]");
		}
